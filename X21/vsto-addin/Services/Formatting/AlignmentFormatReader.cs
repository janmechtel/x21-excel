...
{
    /// <summary>
    /// Reads alignment formatting from a unified snapshot (CPU-only).
    /// </summary>
    public class AlignmentFormatReader : IFormatReader
    {
        public string Name => nameof(AlignmentFormatReader);
        public IReadOnlyCollection<string> SupportedProperties => _supportedProps;
        private static readonly string[] _supportedProps = { "alignment" };

        public void ComputeFormats(FormatSnapshot snapshot, Dictionary<string, FormatSettings> formattedCells)
        {
            if (snapshot.Alignment == null) return;
            ...
            for (var r = 0; r < rows; r++)
            {
                ...
                {
                    var alignment = FormatReaderHelpers.ConvertAlignment(snapshot.Alignment[r, c]);
